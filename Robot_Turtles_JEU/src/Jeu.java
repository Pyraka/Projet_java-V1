...
public class Jeu {
    ...
    public static ArrayList<Joueur> listJoueurs = new ArrayList<> ();
                    ...
                    switch (choix) {
                        case 1:
                            ...
                            for (int j = 0; j < 5; j++) { // compteur j permet d'indiquer toutes les cartes du joueur
                                System.out.println (listJoueurs.get (niemeJoueur).getHand ().get (j).getColor ());

                            }
                            System.out.println (" ");
                            listJoueurs.get (niemeJoueur).completeAlgorithm (); // complete l'algorithme  pour le joueuer qui est en train de joueur
                            listJoueurs.get (niemeJoueur).setCurrentAlgorithm (listJoueurs.get (niemeJoueur).getCurrentAlgorithm ());
                            break;
                        case 2:
                            plateau.poseBloc (listJoueurs.get (niemeJoueur));
                            break;
                        case 3:
                            for (int niemeCarte = 0; niemeCarte < listJoueurs.get (niemeJoueur).getCurrentAlgorithm ().size (); niemeCarte++) {
                                switch (listJoueurs.get (niemeJoueur).getCurrentAlgorithm ().get (niemeCarte).getColor ()) {
                                    case "bleue":
                            ...
                            listJoueurs.get (niemeJoueur).clearAlgorithm ();
                            break;
                    ...
                    switch (nbJoueur) {
                        case 2:
                            if (listJoueurs.size () >= 2) {
                                listJoueurs.get (niemeJoueur).setDefausseCarte ();
                                listJoueurs.get (niemeJoueur).setHand (listJoueurs.get (niemeJoueur).getHand ()); // permet de mettre en mémoire la main du joueur
                                System.out.println (" ");
                            }
                            break;
                        case 3:
